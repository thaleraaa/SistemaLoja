...
class BTree:
    ...
    def buscar_chave(self, k, x=None):
        if x is not None:
            i = 0
            while i < len(x.chaves) and int(k) > int(x.chaves[i][0]):  # Converta k e x.chaves[i][0] para int
                i += 1
            if i < len(x.chaves) and int(k) == int(x.chaves[i][0]):  # Converta k e x.chaves[i][0] para int
                return (x, i)
            elif x.folha:
                return None
            else:
