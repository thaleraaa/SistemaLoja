    ...
    def generate_ordered_partitions(self, input_filename, output_filename, M):
        ...
        for index, line in df.iterrows():
            ...
            if A < M:
                ...
                if len(memoria) != M: 
                     ...
                     if new_line["Product Code"] < min_product_code:
                        ...
                        if len(reservatorio) == M:  
                            ...
                            for a in memoria:
                                ...
                                output_file.write(f"{a['Product Name']},{a['Product Code']},{a['Quantity']},{a['Price']}\n")                 #
                            output_file.close()
                    ...
                    min_product_code = min(memoria, key=lambda x: x["Product Code"])["Product Code"]
                    # pega o valor minimo da memoria, A LINHA INTEIRA
                    min_product_code_row = min(memoria, key=lambda x: x["Product Code"])
                    ...
                    output_file.write(f"{min_product_code_row['Product Name']},{min_product_code_row['Product Code']},{min_product_code_row['Quantity']},{min_product_code_row['Price']}\n")

                    # Remove o valor minimo da memoria
                    min_index = min(range(len(memoria)), key=lambda i: memoria[i]["Product Code"])
        ...
        if ultimo_indice == index:
            ...
            if len(reservatorio) != 0:                                                  #  
                reservatorio = sorted(reservatorio, key=lambda x: x["Product Code"])                                                    #
                partition_number += 1                                                   #
                ...
                for b in reservatorio:                                                  #
                    output_file.write(f"{b['Product Name']},{b['Product Code']},{b['Quantity']},{b['Price']}\n")                                       #
            #---------------------------------------------------------------------------#
            output_file.close()                                                         
